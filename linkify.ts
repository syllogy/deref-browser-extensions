// https://github.com/eight04/linkify-plus-plus-core
...
class Linkifier extends Events {
    ...
    var next = () => {
      ...
      if (now - startTime > this.options.timeout) {
        this.emit(
          'error',
          new Error(
            `max execution time exceeded: ${now - startTime}, on ${this.root}`,
          ),
        );
        return;
      }
  ...
  *generateChunks() {
  ...
  buildLink(result, content) {
    ...
    if (
      embedImage &&
      /^[^?#]+\.(?:jpg|jpeg|png|apng|gif|svg|webp)(?:$|[?#])/i.test(result.url)
    ) {
      child = new Image();
...
export { INVALID_TAGS, Linkifier, linkify };
